...
docker compose down -v           # ВКЛ. удалить volume'ы (postgres + minio)
```
...
## Демонстрация на зачёте (мини-сценарий)

1. `docker compose up -d --build`
2. В Telegram: `/start` от **двух разных** аккаунтов → заполнить анкеты с фото
3. Открыть **MinIO console** (http://localhost:9001) → бакет `photos` → видим загруженные файлы под `users/<tg_id>/...`
4. В аккаунте **A** «🔍 Смотреть анкеты» → лайк на **B**
5. В аккаунте **B** «🔍 Смотреть анкеты» → лайк на **A** → оба получают уведомление о мэтче
6. **A** жмёт «💬 Написать первым» → **B** получает уведомление, рейтинг **A** растёт за начатый диалог
7. Открыть **RabbitMQ management** (http://localhost:15672) → exchange `dating.events` → видим события
8. `curl http://localhost:8000/metrics` → видим HTTP метрики FastAPI
9. `curl http://localhost:8000/profile/<tg_id>` → видим разложение рейтинга по уровням
10. **Реферальная ссылка**: в Настройках взять ссылку `?start=ref_<id>`, открыть с третьего аккаунта → у пригласившего `referrals_count++` и пересчёт рейтинга
...
|---|---|
| Уровень 1: рейтинг по анкете, полнота, фото | ✅ `calculate_primary_score` |
| Уровень 1: первичные предпочтения | ✅ `preferred_*` поля + `compatibility_bonus` |
| Уровень 2: количество лайков | ✅ |
| Уровень 2: like/skip ratio | ✅ |
| Уровень 2: частота мэтчей | ✅ |
| Уровень 2: частота инициирования диалогов | ✅ `dialog_started` + `/matches/dialog-started` |
| Уровень 2: активность по времени суток | ✅ таблица `user_activity_hourly` + бонус за пиковый час |
| Уровень 3: комбинированный рейтинг с весами | ✅ `combine_scores` (0.45/0.35/0.10/0.10) |
| Уровень 3: реферальная система | ✅ deep-link `ref_<id>` + `referrals_count` + `calculate_referral_score` |
| Отдельная таблица рейтингов | ✅ `user_ratings` |
| Регулярный пересчёт через Celery | ✅ `celery beat` каждый час |
| Redis: очередь предотранжированных анкет | ✅ `queue:{tg_id}` |
| MQ для событий | ✅ RabbitMQ topic exchange `dating.events` |
| Метрики | ✅ Prometheus `/metrics` |
| Логирование | ✅ structured stdout |
| S3 для изображений | ✅ MinIO + boto3 + presigned URL |
| /start использует Telegram ID | ✅ |
...
## Известные ограничения / куда можно расти
- alembic не подключён — используются идемпотентные `ALTER TABLE IF NOT EXISTS` при старте backend
- активность по часам — глобальная (не разделена по дням), достаточно как proof-of-concept
- presigned URL у MinIO выдаются с публичного endpoint — для прода понадобится reverse-proxy
- тестов нет — для зачёта приоритет был на функциональности и архитектуре
